...
fn test_llm_total_parameters() {
    ...
    let num_heads = 8;
    let embeddings = Box::new(Embeddings::new(vocab.clone()));
    let transformer_block = Box::new(TransformerBlock::new(EMBEDDING_DIM, HIDDEN_DIM, num_heads));
    let output_projection = Box::new(OutputProjection::new(EMBEDDING_DIM, vocab_size));
    ...
    let expected_transformer_block_parameters = (2 * EMBEDDING_DIM) + // LayerNorm 1
    (4 * EMBEDDING_DIM * EMBEDDING_DIM) + // Multi-Head SelfAttention (Q, K, V, O projection matrices)
    (2 * EMBEDDING_DIM) + // LayerNorm 2

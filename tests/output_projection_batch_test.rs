use llm::{BATCH_SIZE, EMBEDDING_DIM, BatchLayer, BatchOutputProjection};
use ndarray::Array3;
...
fn test_output_projection_creation() {
    let vocab_size = 10;
    let output_proj = BatchOutputProjection::new(EMBEDDING_DIM, vocab_size);
...
fn test_output_projection_forward() {
    let vocab_size = 10;
    let mut output_proj = BatchOutputProjection::new(EMBEDDING_DIM, vocab_size);
    ...
    assert_eq!(output.shape(), [BATCH_SIZE, 3, vocab_size]);
}
...
fn test_output_projection_backward() {
    ...
    let grads = Array3::ones((BATCH_SIZE, 3, vocab_size));

    // Test backward pass
    let grad_input = output_proj.backward(&grads, 0.01);

    // Check gradient input shape
    assert_eq!(grad_input.shape(), [BATCH_SIZE, 3, EMBEDDING_DIM]);
...
fn test_output_projection_training() {
    ...
    for _ in 0..5 {
        ...
        let mut grads = Array3::zeros((BATCH_SIZE, 3, vocab_size));
        grads[[0,0,0]] = 1.0; // Set gradient for first token
